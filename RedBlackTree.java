...
public class RedBlackTree<T extends Comparable<T>, S> {
	...
	private final Node<T, S> NIL = new Node<T, S>(null, null, null, BLACK, null, null);
	...
	public boolean insert(T key, S object) {
		...
		Node<T, S> newElement = new Node<T, S>(NIL, NIL, parent, RED, key, object);
	...
	private void insert_fix1(Node<T, S> element) {
	...
	private void insert_fix2(Node<T, S> element) {
	...
	private void insert_fix3(Node<T, S> element) {
	...
	private void insert_fix4(Node<T, S> element) {
	...
	private void insert_fix5(Node<T, S> element) {
		element.getParent().setColor(BLACK);
		grandparent(element).setColor(RED);
	...
	public S delete(T key) {
		if (searchNode(key) != null) {
			Node<T, S> maxValue, child, element;
			element = searchNode(key);
			S object = element.getObject();
	...
	private void delete_fix1(Node<T, S> element) {
		if (element.getParent() == null) {
			return;
		} else {
			delete_fix2(element);
	...
	private void delete_fix2(Node<T, S> element) {
	...
	private void delete_fix3(Node<T, S> element) {
		if (element.getParent().getColor() == BLACK
	...
	private void delete_fix4(Node<T, S> element){
		if (element.getParent().getColor() == RED
				&& sibling(element).getColor() == BLACK
				&& sibling(element).getLeftChild().getColor() == BLACK
				&& sibling(element).getRightChild().getColor() == BLACK)
	...
	private void delete_fix5(Node<T, S> element){
	...
	private void delete_fix6(Node<T, S> element){
	...
	private void leftRotate(Node<T, S> rotationElement) {

		Node<T, S> rightElement = rotationElement.getRightChild();
	...
	private void rightRotate(Node<T, S> rotationElement) {

		Node<T, S> leftElement = rotationElement.getLeftChild();
	...
	private Node<T, S> grandparent(Node<T, S> element) {
		return element.getParent().getParent();
	}

	private Node<T, S> uncle(Node<T, S> element) {
	...
	private Node<T, S> sibling(Node<T, S> element) {
	...
	private Node<T, S> searchNode(T key){
		Node<T, S> current = root;
		while (current != NIL){
			if (current.getKey().equals(key)){
				return current;
	...
	public ArrayList<Node<T,S>> getList(){
		ArrayList<Node<T,S>> arrayList = new ArrayList<Node<T,S>>();
		getList(root, arrayList);
		return arrayList;
	}
	private void getList(Node<T, S> element, ArrayList<Node<T,S>> arrayList){
		if (element.getLeftChild() != NIL) { 
			getList(element.getLeftChild(), arrayList);
		}
		
		arrayList.add(element);
	
		if (element.getRightChild() != NIL) {
			getList(element.getRightChild(), arrayList);
		}
	}
	...
	private int size(Node<T, S> element) { 
	  if (element == NIL) {
		  return(0); 
	  } else { 
	    return(size(element.getLeftChild()) + 1 + size(element.getRightChild())); 
	  } 
	} 

	private void inOrder(Node<T, S> element, int depth){
		if (element.getLeftChild() != NIL) { 
			inOrder(element.getLeftChild(), depth + 1);
		}
		
		System.out.print(element.toString() + "|" + depth + "   ");
	...
	private void preOrder(Node<T, S> element, int depth){
		System.out.print(element.toString() + "|" + depth + "   ");
		if (element.getLeftChild() != NIL) {
	...
	private void postOrder(Node<T, S> element, int depth) {
		if (element.getLeftChild() != NIL) {
			inOrder(element.getLeftChild(), depth + 1);
		}
		if (element.getRightChild() != NIL) {
